...
struct node *mergeLL(struct node *tmp1, struct node *tmp2, struct node *tmp3)
{
	...
	if(tmp1->data && tmp2->data)
	{
		while(1)
		{
			...
			// tmp3->next = (struct node *) malloc(sizeof(struct node));		X
			// tmp3 = tmp3->next;		X
		}

		// printf("\n\ntmp1: %d, tmp2: %d, tmp3: %d\n",tmp1->data,tmp2->data,tmp3->data);
		// printf("tmp1: %p, tmp2: %p, tmp3: %p\n",tmp1->next,tmp2->next,tmp3->next);

    	if(!(tmp2->next) && tmp2->data > tmp1->data) tmp3->next = tmp2;
		else if(!(tmp2->next)) tmp3->next = tmp1;
		else tmp3->next = tmp2;
